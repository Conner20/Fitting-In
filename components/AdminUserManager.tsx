...
<button type="button" data-confirming={deleteArmed?"true":"false"} onClick={()=>void (selected.every(id=>users.find(user=>user.id===id)?.deletedAt)?recover():remove())} disabled={selected.length===0} aria-disabled={selected.length===0} className={`flex h-9 min-w-0 flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border px-2 text-xs font-bold disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-35 md:flex-none md:px-3 ${selected.every(id=>users.find(user=>user.id===id)?.deletedAt)&&selected.length?"border-[#22c55e] text-[#22c55e]":"border-[#e6a0a0] text-[#d06b6b]"}`}>
{selected.every(id=>users.find(user=>user.id===id)?.deletedAt)&&selected.length?<RotateCcw aria-hidden="true" className="h-3.5 w-3.5 shrink-0"/>:<Trash2 aria-hidden="true" className="h-3.5 w-3.5 shrink-0"/>}{deleteArmed?"Are you sure?":selected.every(id=>users.find(user=>user.id===id)?.deletedAt)&&selected.length?"Recover account":"Delete selected"}</button>
