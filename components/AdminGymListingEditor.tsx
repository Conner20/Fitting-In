...
function StructuredChecklist({options,selected,onChange,otherLabel}:{options:string[];selected:string[];onChange:(items:string[])=>void;otherLabel:string}){const normalizedOptions=new Set(options.map(option=>option.toLowerCase()));const custom=selected.filter(item=>!normalizedOptions.has(item.toLowerCase()));const toggle=(option:string)=>onChange(selected.some(item=>item.toLowerCase()===option.toLowerCase())?selected.filter(item=>item.toLowerCase()!==option.toLowerCase()):[...selected,option]);const optionClass="inline-flex w-fit max-w-full cursor-pointer items-center gap-2 rounded-xl border px-3 py-2 text-sm transition";return <div className="mt-2 min-w-0 space-y-3"><div className="flex min-w-0 flex-wrap items-start gap-2">{options.map(option=>{const checked=selected.some(item=>item.toLowerCase()===option.toLowerCase());return <label key={option} className={`${optionClass} ${checked?"border-emerald-600 bg-emerald-50 dark:border-emerald-500 dark:bg-emerald-950":"border-zinc-200 bg-white dark:border-white/10 dark:bg-white/5"}`}><input type="checkbox" checked={checked} onChange={()=>toggle(option)} className="h-4 w-4 shrink-0 accent-[#22c55e]"/><span className="min-w-0 break-words">{option}</span></label>})}<label className={`${optionClass} ${custom.length?"border-emerald-600 bg-emerald-50 dark:border-emerald-500 dark:bg-emerald-950":"border-zinc-200 bg-white dark:border-white/10 dark:bg-white/5"}`}><input type="checkbox" checked={custom.length>0} onChange={(event)=>onChange(event.target.checked?[...selected,"Other"]:selected.filter(item=>normalizedOptions.has(item.toLowerCase())))} className="h-4 w-4 shrink-0 accent-[#22c55e]"/><span className="min-w-0 break-words">{otherLabel}</span></label></div>{custom.length>0&&<textarea required rows={2} value={custom.filter(item=>item!=="Other").join("\n")} onChange={(event)=>onChange([...selected.filter(item=>normalizedOptions.has(item.toLowerCase())),...lines(event.target.value)])} className="w-full min-w-0 max-w-full rounded-xl border border-zinc-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-emerald-600 dark:border-white/10 dark:bg-zinc-900" placeholder="Enter one custom item per line"/>}</div>}
