...
function TimeInput({label,value,onChange}:{label:string;value:string;onChange:(value:string)=>void}){return <span className="relative min-w-0 flex-1"><input aria-label={label} type="time" value={value} onChange={event=>onChange(event.target.value)} className="gym-time-input w-full min-w-0 rounded-lg border border-zinc-200 bg-white px-2 py-2 pr-8 text-sm dark:border-white/10 dark:bg-zinc-900 dark:text-white sm:py-1.5"/><Clock3 aria-hidden="true" className="gym-time-input-icon pointer-events-none absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-500 dark:text-zinc-200"/></span>}
