...
export default function GymHoursEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
    ...
    return <div className="mt-2 overflow-hidden rounded-xl border border-zinc-200 dark:border-white/10">
        {schedule.map((day, index) => <div key={day.day} className="grid items-center gap-2 border-b border-zinc-200 p-3 last:border-b-0 dark:border-white/10 sm:grid-cols-[90px_70px_70px_1fr]">
            ...
            {day.open && !day.allDay ? <div className="flex items-center gap-2"><input aria-label={`${day.day} opening time`} type="time" value={day.opens} onChange={(event) => change(index, { opens: event.target.value })} className="min-w-0 flex-1 rounded-lg border border-zinc-200 bg-white px-2 py-1.5 text-sm dark:border-white/10 dark:bg-zinc-900"/><span className="text-zinc-400">–</span><input aria-label={`${day.day} closing time`} type="time" value={day.closes} onChange={(event) => change(index, { closes: event.target.value })} className="min-w-0 flex-1 rounded-lg border border-zinc-200 bg-white px-2 py-1.5 text-sm dark:border-white/10 dark:bg-zinc-900"/></div> : <span className="text-xs text-zinc-500">{day.open ? "Open all day" : "Closed"}</span>}
        </div>)}
