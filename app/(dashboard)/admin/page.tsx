...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  ...
  const [users, gymUsers, dayPassClicks, confirmedDayPasses, demandEvents] = await Promise.all([
    ...
    db.landingEvent.findMany({ where: { eventType: { in: ["GYM_OPENED", "DAY_PASS_CLICKED"] }, ...createdAt }, select: { eventType: true, userId: true, visitorId: true } }),
  ]);
  ...
  const cards: readonly (readonly [string, number | string])[] = [["Users", users], ["Gym Users", gymUsers], ["Day-pass clicks", dayPassClicks], ["Confirmed day passes", confirmedDayPasses], ["Day-pass click rate", dayPassClickRate]];
