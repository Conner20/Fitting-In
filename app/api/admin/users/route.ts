...
export async function GET(request:Request){
  ...
  const [users,events]=await Promise.all([
    ...
    db.landingEvent.findMany({where:{eventType:{in:["DAY_PASS_CLICKED","DAY_PASS_CLAIM_CONFIRMED","DAY_PASS_SIGNUP","MEMBERSHIP_CLICKED","MEMBERSHIP_CLAIM_CONFIRMED","MEMBERSHIP_SIGNUP"]}},select:{eventType:true,userId:true,visitorId:true},take:100_000}),
  ]);
  ...
  for(const event of events){const userId=event.userId||visitorUsers.get(event.visitorId);if(!userId)continue;const totals=activity.get(userId)||{dayPassClicks:0,confirmedDayPasses:0,membershipClicks:0,confirmedMemberships:0};if(event.eventType==="DAY_PASS_CLICKED")totals.dayPassClicks++;if(event.eventType==="DAY_PASS_CLAIM_CONFIRMED")totals.confirmedDayPasses++;if(event.eventType==="MEMBERSHIP_CLICKED")totals.membershipClicks++;if(event.eventType==="MEMBERSHIP_CLAIM_CONFIRMED")totals.confirmedMemberships++;activity.set(userId,totals)}
