...
export default async function GymInvitePage({ params, searchParams }: Props) {
    ...
    if (invite?.gym.isVerified && claim !== "1") return <main className="grid min-h-screen place-items-center bg-zinc-50 p-6 text-zinc-950 dark:bg-neutral-950 dark:text-white"><div className="w-full max-w-md rounded-3xl border border-black/10 bg-white p-8 text-center shadow-sm dark:border-white/10 dark:bg-white/5"><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-[#22c55e]/15 text-2xl text-[#22c55e]">✓</div><h1 className="mt-5 text-2xl font-black">Listing already verified</h1><p className="mt-3 text-sm leading-6 text-zinc-500">The listing for {invite.gym.name} has already been verified. No additional verification is required.</p><StableExploreLink className="gym-invite-explore-action mt-6 inline-flex rounded-full bg-[#22c55e] px-5 py-3 text-sm font-black text-black transition" /></div></main>;
    if (!invite || invite.usedAt || invite.expiresAt <= new Date()) return <main className="grid min-h-screen place-items-center bg-zinc-50 p-6 dark:bg-neutral-950"><div className="max-w-md rounded-3xl border bg-white p-8 text-center dark:border-white/10 dark:bg-white/5"><h1 className="text-2xl font-bold">Invitation unavailable</h1><p className="mt-3 text-sm text-zinc-500">This gym invitation is invalid, expired, or has already been used.</p><StableExploreLink className="gym-invite-explore-action mt-6 inline-flex rounded-full bg-emerald-700 px-5 py-3 text-sm font-semibold text-white" /></div></main>;
