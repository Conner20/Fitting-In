...
export default async function GymInvitePage({ params, searchParams }: Props) {
    ...
    const invite = await db.gymInvite.findUnique({
        ...
        include: { gym: { include: { claims: { where: { status: "PENDING" }, select: { id: true } } } } },
    });
    if (invite && (invite.gym.claims.length > 0 || (invite.proposedData && claim !== "1"))) return <main className="grid min-h-screen place-items-center bg-zinc-50 p-6 text-zinc-950 dark:bg-neutral-950 dark:text-white"><div className="w-full max-w-md rounded-3xl border border-black/10 bg-white p-8 text-center shadow-sm dark:border-white/10 dark:bg-white/5"><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-amber-500/15 text-2xl text-amber-500">⌛</div><h1 className="mt-5 text-2xl font-black">Awaiting admin approval</h1><p className="mt-3 text-sm leading-6 text-zinc-500">The information for {invite.gym.name} has already been submitted for review. It cannot be verified again while admin approval is pending.</p><Link href="/" className="mt-6 inline-flex rounded-full bg-[#22c55e] px-5 py-3 text-sm font-black text-black transition hover:bg-black hover:text-[#22c55e]">Explore Fitting In</Link></div></main>;
